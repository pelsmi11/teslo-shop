...
const updateUsers = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  if (!validRoles.includes(role)) {
    return res.status(401).json({ message: "Rol no permitido" });
  }
  ...
  if (!user) {
    await db.disconnect();
    return res.status(404).json({ message: `Not exist this user ${userId}` });
  }
  user.role = role;
  await user.save();
  await db.disconnect();
  ...
  return res
    .status(200)
    .json({ message: `User ${userId} updated successfully whit role ${role}` });
};
